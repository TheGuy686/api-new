...
@EActionInput()
export default class CreateUpdateStoreServiceConfigActionInput extends BaseActionInput {
	@String({
	...
	@String({
		required: true,
		input: (adm: ActionDataManager) => adm.request.requestBody?.sgId,
		message: 'ServiceConfig "sgId" was not set',
	})
	sgId: string;
